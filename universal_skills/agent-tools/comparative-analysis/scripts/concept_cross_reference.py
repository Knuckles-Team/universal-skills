...
if not os.environ.get("OPENAI_API_KEY") and not os.environ.get("LLM_API_KEY"):
    print(
        "Warning: OPENAI_API_KEY / LLM_API_KEY environment variable is missing.",
        file=sys.stderr,
    )
...
def cross_reference_concept(
    ...
    """
    relevant_chunks = [
        c for c in article_chunks if (c.get("score") or 0.0) >= threshold
    ]
...
def main():
    ...
    target_files_input = input(
        "Enter specific target paths/files to filter by (comma separated), or leave blank for all: "
    ).strip()
    user_target_paths = (
        [p.strip() for p in target_files_input.split(",")] if target_files_input else []
    )
