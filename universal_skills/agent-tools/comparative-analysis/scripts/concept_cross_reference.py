...
try:
    ...
    # Attempt to load from the primary workspace if present
    load_dotenv("/home/apps/workspace/agent-packages/agent-utilities/.env")
except ImportError:
    pass

# Interactive check for required environment variables
if not os.environ.get("LLM_BASE_URL"):
    print("Warning: LLM_BASE_URL environment variable is missing.", file=sys.stderr)
    val = input("Enter LLM_BASE_URL (default: http://localhost:1234/v1): ").strip()
    os.environ["LLM_BASE_URL"] = val if val else "http://localhost:1234/v1"


if not os.environ.get("OPENAI_API_KEY") and not os.environ.get("LLM_API_KEY"):
    print("Warning: OPENAI_API_KEY / LLM_API_KEY environment variable is missing.", file=sys.stderr)
    val = input("Enter API Key (default: Test-1234): ").strip()
...
def cross_reference_concept(
    ...
    """
    relevant_chunks = [c for c in article_chunks if (c.get("score") or 0.0) >= threshold]
    ...
    for chunk in relevant_chunks:
        ...
        paper_path = chunk.get("target_path") or metadata.get("file_path", "unknown")
    ...
    for paper_path, chunks in paper_groups.items():
        ...
        avg_score = sum((c.get("score") or 0.0) for c in chunks) / len(chunks)
        max_score = max((c.get("score") or 0.0) for c in chunks)
...
def main():
    ...
    parser.add_argument(
        ...
        help="Minimum similarity score for paper-concept matches (default: 0.2)."
    )
    parser.add_argument(
    ...
    target_files_input = input("Enter specific target paths/files to filter by (comma separated), or leave blank for all: ").strip()
    user_target_paths = [p.strip() for p in target_files_input.split(",")] if target_files_input else []
    ...
    try:
        ...
        for i, concept in enumerate(remaining):
            ...
            try:
                results = engine.search_hybrid(
                    ...
                    target_paths=user_target_paths
                )
                # Filter to Article nodes only
                article_results = [
                    r for r in results
                    if (r.get("type") or "").lower() in ("article", "document")
                ]
        ...
        # Prevent DuckDB core dump on exit by explicitly disconnecting
        try:
            engine.disconnect()
        except Exception:
            pass
