...
def get_default_workflows_root() -> Path:
    ...
    return Path("/home/apps/workspace/agent-packages/skills/universal-skills/universal_skills/workflows")
...
def parse_workflow_skill(skill_md_path: Path) -> Optional[Dict]:
        ...
        if len(parts) >= 3:
            ...
            if yaml:
                try:
                    ...
                    print_premium(f"⚠️ Warning: YAML parsing error in {skill_md_path}: {e}", YELLOW)
            else:
    ...
    for i, match in enumerate(matches):
        ...
        if depends_raw:
            ...
            if depends_match:
                depends_on = [d.strip() for d in depends_match.group(1).split(",") if d.strip()]
...
def query_local_kg(db_path: Path) -> List[Tuple]:
    ...
    try:
        ...
        if "nodes" in tables:
            ...
            print_premium(f"🤖 Connected to local KG database. Found {len(results)} sample nodes:", BLUE)
            for row in results:
                print(f"  - [{row[1]}] ID: {row[0]}")
        else:
            print_premium("🤖 Connected to SQLite, but did not recognize standard graph-os schema.", YELLOW)
...
def handle_scaffold_flow(args):
    ...
    print_premium(f"🚀 Preparing to scaffold workflow: '{args.name}' under '{args.domain}' domain...", BOLD + BLUE)
    ...
    if not args.interactive and not args.steps_json:
        ...
        while True:
            ...
            component = input("Enter component/tool (e.g. portainer-mcp, user-interaction) or press Enter to finish: ").strip()
            if not component:
                if step_idx == 0:
            ...
            expected = input("Enter expected output/variables (e.g. endpoint_id): ").strip()
            deps_raw = input("Enter dependencies as comma-separated step numbers (e.g. 0,1) [None]: ").strip()
            
            depends_on = []
            if deps_raw:
                depends_on = [f"Step {d.strip()}" for d in deps_raw.split(",") if d.strip()]
    ...
    tags = [t.strip() for t in args.tags.split(",") if t.strip()] if args.tags else ["workflow"]
    requires = [r.strip() for r in args.requires.split(",") if r.strip()] if args.requires else []
    ...
    if dest:
        print_premium(f"\n🎉 Successfully created and registered workflow at {dest}!", BOLD + GREEN)
...
def handle_list_flow(args):
    ...
    print_premium(f"Found {len(workflows)} active workflow-based skills:\n", BOLD + GREEN)
    ...
    for dom, wfs in by_domain.items():
        ...
        for wf in wfs:
            ...
            for s in wf["steps"]:
                dep_str = f" (Depends: {', '.join(s['depends_on'])})" if s["depends_on"] else ""
                print(f"      * Step {s['step']}: [{s['component']}] -> {s['description']}{dep_str}")
        print()
...
def handle_validate_flow(args):
    ...
    if is_valid:
        print_premium("✅ Workflow passes topological DAG checks! No circular dependencies.", GREEN)
    else:
...
def handle_query_kg_flow(args):
    ...
    db_path = Path(args.db) if args.db else Path("/home/apps/workspace/knowledge_graph_test.db")
...
def main():
    ...
    # Scaffold command
    scaffold_parser = subparsers.add_parser("scaffold", help="Scaffold a new workflow skill")
    scaffold_parser.add_argument("name", help="Name of the workflow (e.g. check_disk)")
    scaffold_parser.add_argument("--domain", required=True, help="Domain directory (e.g. infra)")
    scaffold_parser.add_argument("--description", required=True, help="High level description")
    scaffold_parser.add_argument("--tags", help="Comma-separated tags list")
    scaffold_parser.add_argument("--requires", help="Comma-separated required tools list")
    scaffold_parser.add_argument("--root", help="Custom path to the workflows directory")
    scaffold_parser.add_argument("--interactive", action="store_true", help="Interactively enter step details")
    scaffold_parser.add_argument("--steps-json", help="JSON string representing workflow steps")
    ...
    query_parser = subparsers.add_parser("query-kg", help="Query local Knowledge Graph SQLite")
