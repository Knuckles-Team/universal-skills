...
def generate_crontab_entries(repo_root: Optional[Path] = None) -> str:
    ...
    lines.append(
        "# Set PATH for cron jobs (include user-local bins for CLIs like codex)"
    )
    lines.append(
        "PATH=$HOME/.local/bin:$HOME/bin:/usr/local/bin:/opt/homebrew/bin:/usr/bin:/bin"
    )
    lines.append("")
...
def list_heartbeats_formatted() -> str:
    ...
    for hb in heartbeats:
        ...
        max_runtime = hb.get("max_runtime", "")
        session_mode = (
            str(hb.get("session_mode", "restore") or "restore").strip().lower()
        )
        details = []
        if max_runtime:
