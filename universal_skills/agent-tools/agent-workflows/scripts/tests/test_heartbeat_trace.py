...
class HeartbeatTraceTests(unittest.TestCase):
    ...
    def test_append_and_read_audit_events(self):
        ...
        events = main._read_heartbeat_audit_events(
            self.temp_root, agent_id="emp-0001", limit=10
        )
        self.assertEqual(len(events), 2)
        self.assertEqual(events[0]["hb_id"], "20260209-120002")
        self.assertEqual(events[1]["hb_id"], "20260209-120001")

        by_hb = main._read_heartbeat_audit_events(
            self.temp_root, heartbeat_id="20260209-120001", limit=10
        )
    ...
    def test_trace_limit_is_bounded(self):
        ...
        events = main._read_heartbeat_audit_events(
            self.temp_root, agent_id="emp-0002", limit=5
        )
    ...
    def test_read_filters_since_until(self):
        ...
        events = main._read_heartbeat_audit_events(
            self.temp_root, agent_id="emp-0005", since=since, until=until, limit=10
        )
    ...
    def test_classify_heartbeat_ack(self):
        self.assertEqual(
            main._classify_heartbeat_ack(
                last_state="idle", timed_out=False, waited_for_ack=True
            ),
            ("ack", ""),
        )
        self.assertEqual(
            main._classify_heartbeat_ack(
                last_state="blocked", timed_out=False, waited_for_ack=True
            ),
            ("blocked", "blocked"),
        )
        self.assertEqual(
            main._classify_heartbeat_ack(
                last_state="busy", timed_out=True, waited_for_ack=True
            ),
            ("timeout", "timeout"),
        )
        self.assertEqual(
            main._classify_heartbeat_ack(
                last_state="busy", timed_out=False, waited_for_ack=True
            ),
            ("no_ack", "no_ack"),
        )
        self.assertEqual(
            main._classify_heartbeat_ack(
                last_state=None, timed_out=False, waited_for_ack=False
            ),
            ("not_checked", ""),
        )
    ...
    def test_read_skips_invalid_lines(self):
        ...
        events = main._read_heartbeat_audit_events(
            self.temp_root, agent_id="emp-0003", limit=10
        )
        self.assertEqual(len(events), 1)
