...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Run integration tests with flaky-control retries."
    )
    parser.add_argument(
        "--pattern", default="test_integration_*.py", help="unittest file pattern"
    )
    parser.add_argument(
        "--attempts", type=int, default=2, help="max attempts before failing"
    )
    parser.add_argument(
        "--artifact-dir",
        default=".artifacts/integration",
    ...
    for attempt in range(1, attempts + 1):
        ...
        summary["results"].append(
            {"attempt": attempt, "return_code": rc, "log_file": str(log_file)}
        )
