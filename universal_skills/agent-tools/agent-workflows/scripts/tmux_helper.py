...
def _set_agent_pane_title(agent_id: str) -> None:
    ...
    subprocess.run(
        ["tmux", "select-pane", "-t", container, "-T", title],
        capture_output=True,
        text=True,
    )
...
def start_session_with_layout(
    ...
    *,
    ...
    try:
        ...
        subprocess.run(
            ["tmux", "select-pane", "-t", target_pane], capture_output=True, text=True
        )
        return target_pane
    except Exception:
...
def _is_codex_model_choice_prompt(output: str) -> bool:
    ...
    if "codex just got an upgrade" in lowered:
        return True
    if (
        "choose how you" in lowered
        and "codex" in lowered
        and "try new model" in lowered
        and "use existing model" in lowered
    ):
        return True
    return False
...
def wait_for_agent_ready(agent_id: str, launcher: str, timeout: int = 45) -> bool:
    ...
    while (time.time() - start_time) < timeout:
        ...
        if result.returncode == 0:
            ...
            if (
                is_codex
                and codex_model_prompt_attempts < 3
                and _is_codex_model_choice_prompt(output)
            ):
            ...
            if is_codex:
                for line in output.split("\n"):
                    ...
                    if stripped.startswith(
                        ("›", "❯")
                    ) and not _CODEX_MENU_OPTION_RE.match(stripped):
                        return True
                    ...
                    for line in lines:
                        ...
                        if is_droid:
                            ...
                            if stripped == pattern or (
                                stripped.startswith(pattern) and len(stripped) <= 3
                            ):
                                return True
...
def get_agent_runtime_state(agent_id: str, launcher: str = "") -> Dict[str, object]:
    ...
    if (
        ...
        if (
            ...
            and (now - last_failure) < _CODEX_MODEL_PROMPT_FAILURE_THROTTLE_S
        ):
            return evaluate_runtime_state(
                output=output,
                runtime_config=runtime_config,
...
def wait_for_prompt(agent_id: str, launcher: str, timeout: int = 30) -> bool:
    ...
    sys.path.insert(0, str(Path(__file__).parent.parent))
    from providers import get_prompt_patterns, get_startup_wait
