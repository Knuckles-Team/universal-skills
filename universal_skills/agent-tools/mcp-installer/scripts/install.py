...
def get_tool_paths() -> Dict[str, Path]:
    ...
    appdata = Path(os.environ.get("APPDATA", "~\\AppData\\Roaming")).expanduser() if is_windows else None
    ...
    if is_windows:
        ...
        paths["claude-desktop"] = Path("~/Library/Application Support/Claude/claude_desktop_config.json").expanduser()
    else:
        paths["claude-desktop"] = Path("~/.config/Claude/claude_desktop_config.json").expanduser()
    ...
    if is_windows:
        localappdata = Path(os.environ.get("LOCALAPPDATA", "~\\AppData\\Local")).expanduser()
        agent_utils_path = localappdata / "agent-utilities" / "mcp_config.json"
    elif is_mac:
        agent_utils_path = Path("~/Library/Application Support/agent-utilities/mcp_config.json").expanduser()
    else:
        agent_utils_path = Path("~/.config/agent-utilities/mcp_config.json").expanduser()
...
def merge_mcp_configs(source_data: Dict[str, Any], target_data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if not isinstance(source_servers, dict):
        logger.warning("Source configuration does not have a valid 'mcpServers' object.")
        return merged
...
def install_mcp_config(source_path: Path, target_path: Path, force: bool = False) -> bool:
...
def main():
    ...
    parser.add_argument(
        "--config", 
        required=True,
        help="Path to the source mcp_config.json file"
    )
    parser.add_argument(
        "--tool", 
    ...
    parser.add_argument(
        "--path", 
        help="Explicit custom path to the target configuration file"
    )
    parser.add_argument(
        "--force", 
        action="store_true", 
