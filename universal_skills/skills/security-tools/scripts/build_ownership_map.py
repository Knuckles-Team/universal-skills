        ...
        owners.append(
            {
                ...
                "touches": touches,
                "touch_share": (
                    round(touches / total_touches, 4) if total_touches else 0.0
                ),
                "recency_share": (
                    round(recency / total_recency, 4) if total_recency else 0.0
                ),
                "sensitive_share": (
                    round(sensitive / total_sensitive, 4) if total_sensitive else 0.0
                ),
...
def build_ownership_map(args: argparse.Namespace) -> Path:
    ...
    summary = {
        ...
        "stats": {
            ...
            "cochange_commits_used": (
                cochange_commits_used if not args.no_cochange else 0
            ),
            "cochange_commits_skipped": (
                cochange_commits_skipped if not args.no_cochange else 0
            ),
            "cochange_commits_filtered": (
                cochange_commits_filtered if not args.no_cochange else 0
            ),
            "cochange_files_excluded": (
                cochange_files_excluded if not args.no_cochange else 0
            ),
        },
    }
