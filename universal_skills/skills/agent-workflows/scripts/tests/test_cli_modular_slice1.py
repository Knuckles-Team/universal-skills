...
class CliModularSlice1Tests(unittest.TestCase):
    ...
    def test_heartbeat_wrapper_delegates_to_heartbeat_handler(self):
        ...
        mock_handler.assert_called_once()
        self.assertIs(
            mock_handler.call_args.kwargs["run_handler"], main.cmd_heartbeat_run
        )
        self.assertIs(
            mock_handler.call_args.kwargs["trace_handler"], main.cmd_heartbeat_trace
        )
        self.assertIs(
            mock_handler.call_args.kwargs["slo_handler"], main.cmd_heartbeat_slo
        )

    def test_schedule_run_wrapper_delegates_to_schedule_run_handler(self):
        args = object()
        with patch(
            "main.schedule_run_cmd_schedule_run", return_value=47
        ) as mock_handler:
    ...
    def test_schedule_wrapper_delegates_to_schedule_handler(self):
        ...
        self.assertIs(mock_handler.call_args.kwargs["deps"], main)
        self.assertIs(
            mock_handler.call_args.kwargs["schedule_run_handler"], main.cmd_schedule_run
        )
