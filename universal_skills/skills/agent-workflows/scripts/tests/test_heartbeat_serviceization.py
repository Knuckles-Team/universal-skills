...
class HeartbeatServiceizationTests(unittest.TestCase):
    def test_parse_recovery_policy_service_defaults(self):
        policy = parse_heartbeat_recovery_policy(
            {"enabled": True}, fallback_modes={"none", "fresh"}
        )
    ...
    def test_parse_recovery_policy_normalizes_restart_and_blank_channel(self):
        ...
        policy = parse_heartbeat_recovery_policy(
            heartbeat, fallback_modes={"none", "fresh"}
        )
    ...
    def test_parse_recovery_policy_accepts_cli_overrides(self):
        ...
        policy = parse_heartbeat_recovery_policy(
            {"enabled": True}, args=args, fallback_modes={"none", "fresh"}
        )
    ...
    def test_state_machine_ack_reason_code(self):
        ack, failure, reason = classify_heartbeat_ack(
            waited_for_ack=True, last_state="busy", timed_out=True
        )
        self.assertEqual(
            (ack, failure, reason), ("timeout", "timeout", "HB_ACK_TIMEOUT")
        )
    ...
    def test_failure_reason_code_mapping(self):
        ...
        self.assertEqual(
            failure_reason_code(failure_type="blocked"), "HB_AGENT_BLOCKED"
        )
    ...
    def test_should_retry_uses_state_machine_rules(self):
        self.assertTrue(
            should_retry_heartbeat_attempt(
                failure_type="no_ack", attempt_index=0, max_retries=1
            )
        )
        self.assertFalse(
            should_retry_heartbeat_attempt(
