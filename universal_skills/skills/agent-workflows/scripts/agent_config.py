...
def _build_main_agent_config(
    *,
    ...
    launcher = (
        str(env_vars.get(MAIN_AGENT_LAUNCHER_ENV, "codex") or "").strip() or "codex"
    )
    launcher_args_raw = str(
        env_vars.get(MAIN_AGENT_LAUNCHER_ARGS_ENV, "") or ""
    ).strip()
    ...
    for candidate in [
        ...
        agents_dir
        / MAIN_AGENT_NAME
        / AGENT_DIR_PROFILE_FILENAME,  # agents/main/AGENTS.md
    ]:
...
def resolve_agent(
    name_or_id: str, agents_dir: Optional[Path] = None
...
def list_all_agents(agents_dir: Optional[Path] = None) -> Dict[str, Dict[str, Any]]:
    ...
    if agents_dir is None:
        ...
        repo_root = (
            agents_dir.parent if agents_dir.name == "agents" else get_repo_root()
        )
...
def build_system_prompt(
    ...
    *,
    ...
    if role_definition:
        parts.append(
            f"# {config.get('name', 'Agent').upper()} ROLE\n\n{role_definition}"
        )
...
def get_schedule_task(
    schedule: Dict[str, Any], repo_root: Optional[Path] = None
) -> str:
    """
...
def get_agent_schedule(
    agent_name: str, job_name: str, agents_dir: Optional[Path] = None
) -> Optional[Dict[str, Any]]:
    """
