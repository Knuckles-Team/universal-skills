...
Examples:
  ...
  EOF
    ...
    list_parser.add_argument(
        "--running", "-r", action="store_true", help="Show only running agents"
    )
    ...
    monitor_parser.add_argument(
        "--follow", "-f", action="store_true", help="Follow output (like tail -f)"
    )
    monitor_parser.add_argument(
        "--lines",
        "-n",
    ...
    schedule_subparsers = schedule_parser.add_subparsers(
        dest="schedule_command", help="Schedule commands"
    )

    doctor_parser = subparsers.add_parser(
        "doctor", help="Check environment and configuration"
    )
    doctor_parser.add_argument(
        "--deep", action="store_true", help="Perform deeper checks"
    )

    schedule_subparsers.add_parser("list", help="List all scheduled jobs")

    schedule_sync_parser = schedule_subparsers.add_parser(
        "sync", help="Sync schedules to crontab"
    )
    ...
    schedule_run_parser = schedule_subparsers.add_parser(
        "run", help="Run a scheduled job manually"
    )
    schedule_run_parser.add_argument("agent", help="Agent name")
    schedule_run_parser.add_argument(
        "--job", "-j", required=True, help="Job name to run"
    )
    schedule_run_parser.add_argument(
        "--timeout", "-t", help="Override max runtime (e.g., 30m, 2h)"
    )
    ...
    heartbeat_subparsers = heartbeat_parser.add_subparsers(
        dest="heartbeat_command", help="Heartbeat commands"
    )

    heartbeat_subparsers.add_parser("list", help="List all heartbeat jobs")

    heartbeat_sync_parser = heartbeat_subparsers.add_parser(
        "sync", help="Sync heartbeats to crontab"
    )
    ...
    heartbeat_run_parser = heartbeat_subparsers.add_parser(
        "run", help="Run a heartbeat manually"
    )
    ...
    heartbeat_trace_parser = heartbeat_subparsers.add_parser(
    ...
    heartbeat_trace_parser.add_argument(
        "--since", help="Filter events at/after time (ISO-8601, UTC recommended)"
    )
    heartbeat_trace_parser.add_argument(
        "--until", help="Filter events at/before time (ISO-8601, UTC recommended)"
    )
    heartbeat_trace_parser.add_argument(
        "--limit",
        "-n",
    ...
    heartbeat_trace_parser.add_argument(
        "--json", action="store_true", help="Output records as JSON"
    )

    heartbeat_slo_parser = heartbeat_subparsers.add_parser(
        "slo", help="Summarize heartbeat SLO metrics"
    )
    ...
    heartbeat_slo_parser.add_argument(
        "--since", help="Override start time (ISO-8601, UTC recommended)"
    )
    heartbeat_slo_parser.add_argument(
        "--until", help="Override end time (ISO-8601, UTC recommended)"
    )
    heartbeat_slo_parser.add_argument(
