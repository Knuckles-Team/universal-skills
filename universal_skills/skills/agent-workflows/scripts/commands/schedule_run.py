...
def _resolve_schedule_task_path(
    schedule: dict, repo_root: Path, *, expand_env_vars: Callable[[str], str]
) -> Optional[Path]:
...
def _print_runtime_skip_message(skip_reason: str) -> None:
    ...
    if skip_reason == "stuck_below_threshold":
        print(
            "⏭️  Agent appears stuck but below restart threshold; skipping scheduled task"
        )
        print("   Will retry on next cron execution")
        return
...
def cmd_schedule_run(args, *, deps: Any, start_handler: Callable):
    ...
    if not agent_config.get("enabled", True):
        ...
        print(
            f"⏭️  Agent '{agent_name}' is disabled - skipping scheduled job '{args.job}'"
        )
