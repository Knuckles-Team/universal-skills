...
class EPUBGenerator:
    ...
    def _render_chapter(self, chapter: Chapter) -> str:
        ...
        if chapter.title:
            html_parts.append(
                f'<h1 id="{chapter.anchor}">{self._escape_html(chapter.title)}</h1>'
            )
        ...
        for section in chapter.sections:
            html_parts.append(
                f'<h{section.level} id="{section.anchor}">{self._escape_html(section.title)}</h{section.level}>'
            )
            if section.content:
...
{content}
    ...
    def _write_epub(self, output_path: str) -> None:
        ...
        html_items = [
            item for item in self.book.items if isinstance(item, epub.EpubHtml)
        ]
