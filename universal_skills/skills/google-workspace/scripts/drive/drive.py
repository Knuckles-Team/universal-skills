...
def api_request(
    method: str, endpoint: str, params: Optional[dict] = None, stream: bool = False
) -> dict | bytes:
...
def upload(
    local_path: str, folder_id: Optional[str] = None, name: Optional[str] = None
) -> dict:
    """
...
def copy_file(
    file_id: str, new_name: Optional[str] = None, folder_id: Optional[str] = None
) -> dict:
    """
    Copy a file.
...
def main():
    ...
    search_parser.add_argument(
        "query", nargs="?", help="Search query, URL, or folder ID"
    )
    search_parser.add_argument(
    ...
    list_parser.add_argument(
        "folder_id", nargs="?", help="Folder ID (root if not specified)"
    )
    list_parser.add_argument(
        "--limit", type=int, default=10, help="Max results (default: 10)"
    )
    ...
    upload_parser.add_argument(
        "--name", help="Name for file in Drive (default: local filename)"
    )
