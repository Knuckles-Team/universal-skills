...
def to_utc_iso(dt_string: str) -> str:
    ...
    if "+" in dt_string[-6:] or (
        dt_string[-6:].count("-") > 0 and ":" in dt_string[-6:]
    ):
        return dt_string
...
def get_event(event_id: str, calendar_id: Optional[str] = None) -> dict:
    ...
    return api_request(
        "GET", f"calendars/{urllib.parse.quote(calendar_id, safe='')}/events/{event_id}"
    )
...
def find_free_time(
    attendees: list, time_min: str, time_max: str, duration_minutes: int
) -> dict:
    ...
    request_data = {"timeMin": time_min, "timeMax": time_max, "items": items}
...
def respond_to_event(
    ...
    if response_status not in ["accepted", "declined", "tentative"]:
        return {
            "error": "Invalid response status. Use: accepted, declined, or tentative"
        }
...
def main():
    ...
    create_event_parser.add_argument(
        "start", help="Start time (ISO 8601, e.g., 2024-01-15T10:30:00Z)"
    )
    ...
    create_event_parser.add_argument(
        "--attendees", nargs="+", help="Attendee email addresses"
    )
    ...
    update_event_parser.add_argument(
        "--attendees", nargs="+", help="New attendee email addresses"
    )
    ...
    if args.command == "list-calendars":
        ...
        result = list_events(
            args.calendar, args.time_min, args.time_max, args.max_results
        )
        ...
        result = find_free_time(
            args.attendees, args.time_min, args.time_max, args.duration
        )
    elif args.command == "respond-to-event":
        result = respond_to_event(
            args.event_id, args.response, args.calendar, not args.no_notify
        )
    else:
