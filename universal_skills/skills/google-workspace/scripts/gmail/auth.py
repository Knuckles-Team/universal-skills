...
class OAuthCallbackHandler(http.server.BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if "error" in params:
            ...
            self.end_headers()
            self.wfile.write(
                f"<html><body><h1>Error: {OAuthCallbackHandler.error}</h1></body></html>".encode()
            )
            return
        ...
        if access_token:
            ...
            self.end_headers()
            self.wfile.write(
                b"<html><body><h1>Error: No access token received</h1></body></html>"
            )
...
def perform_oauth_flow() -> Optional[TokenInfo]:
    ...
    state_payload = {"uri": redirect_uri, "manual": False, "csrf": csrf_token}
    ...
    while (
        OAuthCallbackHandler.token_info is None and OAuthCallbackHandler.error is None
    ):
...
def get_valid_access_token(interactive: bool = True) -> Optional[str]:
    ...
    if is_token_expired(token_info) and token_info.refresh_token:
        ...
        if new_tokens:
            token_info.access_token = new_tokens.get(
...
def main():
    ...
    if args.command == "login":
        ...
        if token_info and token_info.access_token:
            ...
            print("Storage: System keyring")
            print(f"Token expired: {expired}")
            if token_info.expires_at:
                expires_in = (
                    (token_info.expires_at - int(time.time() * 1000)) / 1000 / 60
                )
