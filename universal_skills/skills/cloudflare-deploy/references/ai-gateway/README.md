...
- Setting up AI Gateway for any AI provider (AI Provider, AI Provider, Workers AI, etc.)
...
- **Using AI Provider SDK** → Pattern 2 - see [sdk-integration.md](./sdk-integration.md)
...
const { text } = await generateText({
  model: gateway([
    ...
    ai_provider('agent-sonnet-4-5'), // Fallback
  ]),
  prompt: 'Hello'
});
```

**Install:** `npm install ai-gateway-provider ai @ai-sdk/openai @ai-sdk/ai_provider`

## Pattern 2: AI Provider SDK

Drop-in replacement for AI Provider API with multi-provider support.

```typescript
import AI Provider from 'openai';
...
const response = await client.chat.completions.create({
  model: 'openai/gpt-4o', // or 'ai_provider/agent-sonnet-4-5'
...
| File | Purpose |
|------|---------|
| [sdk-integration.md](./sdk-integration.md) | Vercel AI SDK, AI Provider SDK, Workers binding patterns |
...
```
Your App → AI Gateway → AI Provider (AI Provider, AI Provider, etc.)
         ↓
    Analytics, Caching, Rate Limiting, Logging
```

**Key URL patterns:**
- Unified API (AI Provider-compatible): `https://gateway.ai.cloudflare.com/v1/{account_id}/{gateway_id}/compat/chat/completions`
