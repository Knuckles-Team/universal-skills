#!/usr/bin/env python3
"""
Agent Deep Research Skill - Execute autonomous multi-step research tasks.
...
class HistoryManager:
    ...
    def __init__(self, cache_dir: Optional[str] = None):
        ...
        self.cache_dir = Path(
            cache_dir or os.getenv("DEEP_RESEARCH_CACHE_DIR", default_dir)
        )
...
class DeepResearchClient:
    """Client for Agent Deep Research API."""
    ...
    async def stream_research(
        self, query: str, format_spec: Optional[str] = None
                        ...
                        if event_type == "interaction.start":
                            ...
                            if interaction_id:
                                self.history.add_interaction(
                                    interaction_id, query, "streaming"
                                )
...
def print_progress(poll_count: int, elapsed: float, status: str):
    ...
    secs = int(elapsed % 60)
    print(
        f"\r[{mins:02d}:{secs:02d}] Poll #{poll_count} - Status: {status}",
        end="",
        flush=True,
    )
    ...
    try:
        if args.stream:
            ...
            interaction_id = await client.start_research(
                args.query, args.format, previous_id
            )
            ...
            result = await client.wait_for_completion(
                interaction_id, progress_callback=print_progress
            )
...
async def cmd_wait(args):
    ...
    try:
        ...
        result = await client.wait_for_completion(
            args.interaction_id, progress_callback=print_progress
        )
        print()
...
async def cmd_list(args):
    ...
    if args.json:
        ...
        for item in interactions:
            ...
            print(
                f"    Query: {item['query'][:60]}{'...' if len(item['query']) > 60 else ''}"
            )
...
def main():
    parser = argparse.ArgumentParser(
        description="Agent Deep Research - Autonomous multi-step research agent",
...
""",
    ...
    cmd_group.add_argument(
        "--list", "-l", action="store_true", help="List recent research tasks"
    )

    # Query options
    parser.add_argument(
        "--format", "-f", metavar="SPEC", help="Output format specification"
    )
    parser.add_argument(
        "--continue",
        dest="continue_from",
    ...
    parser.add_argument(
        "--stream", action="store_true", help="Stream progress in real-time"
    )
    parser.add_argument(
        "--no-wait",
        action="store_true",
