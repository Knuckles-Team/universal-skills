...
def rule_frontmatter_name_required(skill: dict) -> list[dict]:
    ...
    return [
        {
            "rule": "skill.frontmatter.name_required",
            "category": "frontmatter",
            "severity": "error",
            "penalty": 1.0,
...
def rule_frontmatter_description_required(skill: dict) -> list[dict]:
    ...
    return [
        {
            "rule": "skill.frontmatter.description_required",
            "category": "frontmatter",
            "severity": "error",
            "penalty": 1.0,
...
def rule_frontmatter_name_matches_dir(skill: dict) -> list[dict]:
    ...
    return [
        {
            "rule": "skill.frontmatter.name_matches_directory",
            "category": "frontmatter",
            "severity": "error",
            "penalty": 1.0,
...
def rule_frontmatter_name_slug_format(skill: dict) -> list[dict]:
    ...
    return [
        {
            "rule": "skill.frontmatter.name_slug_format",
            "category": "frontmatter",
            "severity": "error",
            "penalty": 1.0,
...
def rule_frontmatter_name_max_length(skill: dict) -> list[dict]:
    ...
    return [
        {
            "rule": "skill.frontmatter.name_max_length",
            "category": "frontmatter",
            "severity": "error",
            "penalty": 1.0,
...
def rule_frontmatter_field_order(skill: dict) -> list[dict]:
    ...
    return [
        {
            "rule": "skill.frontmatter.field_order",
            "category": "frontmatter",
            "severity": "warn",
            "penalty": 0.5,
...
def rule_frontmatter_unknown_fields(skill: dict) -> list[dict]:
    ...
    return [
        {
            "rule": "skill.frontmatter.unknown_fields",
            "category": "frontmatter",
            "severity": "warn",
            "penalty": 0.5,
...
def rule_description_non_empty(skill: dict) -> list[dict]:
    ...
    return [
        {
            "rule": "skill.description.non_empty",
...
def rule_description_max_length(skill: dict) -> list[dict]:
    ...
    return [
        {
            "rule": "skill.description.max_length",
            "category": "description",
            "severity": "error",
            "penalty": 1.0,
...
def rule_description_use_when_phrase(skill: dict) -> list[dict]:
    ...
    return [
        {
            "rule": "skill.description.use_when_phrase",
            "category": "description",
            "severity": "warn",
            "penalty": 0.5,
...
def rule_description_min_length(skill: dict) -> list[dict]:
    ...
    return [
        {
            "rule": "skill.description.min_recommended_length",
            "category": "description",
            "severity": "warn",
            "penalty": 0.5,
...
def rule_description_anti_trigger(skill: dict) -> list[dict]:
    ...
    if len(desc) > 80:
        return [
            {
                "rule": "skill.description.anti_trigger",
                "category": "description",
                "severity": "warn",
                "penalty": 0.3,
...
def rule_links_local_resolves(skill: dict) -> list[dict]:
    ...
    for match in re.finditer(r"\[([^\]]*)\]\(([^)]+)\)", body):
        ...
        if not resolved.exists():
            findings.append(
                {
                    "rule": "skill.links.local_resolves",
                    "category": "links",
                    "severity": "warn",
                    "penalty": 0.5,
        ...
        if ref_file.is_file() and ref_file.suffix == ".md":
            ...
            if ref_name in body:
                if not ref_file.exists():
                    findings.append(
                        {
                            "rule": "skill.links.references_resolve",
                            "category": "links",
                            "severity": "warn",
                            "penalty": 0.5,
...
def rule_file_trailing_newline(skill: dict) -> list[dict]:
    ...
    if not content.endswith("\n"):
        return [
            {
                "rule": "skill.file.trailing_newline",
                "category": "file_meta",
                "severity": "warn",
                "penalty": 0.5,
    ...
    for desc, paths in descs.items():
        if len(paths) > 1:
            for path in paths:
                findings.append(
                    {
                        "rule": "skill.duplicates.description",
                        "category": "file_meta",
                        "severity": "warn",
                        "penalty": 0.5,
...
def discover_skills(root: Path) -> list[Path]:
    ...
    for pattern in SKILL_DISCOVERY_PATTERNS:
        for match in root.glob(pattern):
            if (
                match.is_file()
                and ".venv" not in str(match)
                and "node_modules" not in str(match)
            ):
