...
def analyze_version_sync(root_dir: str = ".") -> dict:
    ...
    ignore_dirs = {".git", ".venv", "venv", "env", "build", "dist", "node_modules", ".pytest_cache", ".ruff_cache", "__pycache__"}
    ...
    version_pattern = re.compile(rf"(version|VERSION)\b.*?['\"]?{re.escape(current_version)}['\"]?")
    ...
    if drifted_files:
        findings.append(f"Found {len(drifted_files)} file(s) with version '{current_version}' that are NOT tracked in .bumpversion.cfg:")
        ...
        score += 40
        findings.append(f"All version '{current_version}' declarations appear to be tracked correctly.")
        justifications.append({
            "criterion": "version_drift_check",
            "points": 40,
