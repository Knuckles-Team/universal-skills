...
def check_xdg_compliance(project_dir: str | None = None) -> dict[str, Any]:
    ...
    if not failed:
        justifications = ["No non-XDG standard paths found in the Knowledge Graph for this scope."]
    else:
        justifications = ["Found hardcoded legacy home directory paths (~/.appname). Use XDG standard paths instead."]
