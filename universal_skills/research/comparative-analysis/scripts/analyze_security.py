...
def check_security_posture(project_path: Path) -> dict:
    ...
    posture = {
        "security_md": any(
            (project_path / f).exists() for f in ["SECURITY.md", ".github/SECURITY.md"]
        ),
    ...
    for f in project_path.rglob("*.py"):
        ...
        try:
            ...
            if any(
                kw in content
                for kw in ["JWT", "OAuth", "jwt_required", "Depends(", "HTTPBearer"]
            ):
...
def score_security(scan: dict, posture: dict) -> dict:
    ...
    if secrets > 0:
        ...
        details.append(
            f"Hardcoded secrets detected ({secrets}): -{min(secrets * 20, 40)}"
        )
    if findings > 20:
        score -= 30
