...
+ Service), matching the in-repo reference ``agent-utilities/deploy/k8s/graphos.yaml``.
...
def node_affinity(node: str) -> dict[str, Any]:
    """Pin a pod to a node by the homelab ``name=<HOST>`` label."""
    return {
        "nodeAffinity": {
            "requiredDuringSchedulingIgnoredDuringExecution": {
                "nodeSelectorTerms": [
                    {
                        "matchExpressions": [
                            {"key": "name", "operator": "In", "values": [node]}
                        ]
                    }
                ]
            }
...
def _pvcs_and_volumes(
    name: str, volumes: list[str], namespace: str, storage_class: str
...
def to_workload(
    placement: dict[str, Any], namespace: str, storage_class: str
) -> tuple[dict, list[dict]]:
    """Build the workload manifest + any PVCs for one placement."""
    name = placement["service"]
    node = placement["node"]
    kind = select_kind(placement)
    image = placement.get("image", f"registry.arpa/{name}:latest")
...
def emit(blueprint: dict[str, Any], target: str, namespace: str) -> list[dict]:
    ...
    storage_class = (blueprint.get("k8s") or {}).get("storageClass", "local-path")
    namespace = (blueprint.get("k8s") or {}).get("namespace", namespace)
    ...
    for placement in blueprint.get("placements", []):
        if not placement.get("service") or not placement.get("node"):
            continue
        workload, pvcs = to_workload(placement, namespace, storage_class)
