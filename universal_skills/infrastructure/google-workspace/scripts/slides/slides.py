...
def api_request(
    ...
    try:
        ...
        payload, _ = open_json(
            req,
            policy=UrlPolicy(
                frozenset({"slides.googleapis.com", "www.googleapis.com"})
            ),
            timeout=30,
...
def add_slide(
    presentation_id: str, layout: str = "BLANK", insert_at: Optional[int] = None
) -> dict:
    """
    ...
    return api_request(
        "POST",
        f"{SLIDES_API_BASE}/presentations/{pid}:batchUpdate",
        data={"requests": [request]},
    )


def replace_text(
    presentation_id: str, find_text: str, replace_with: str, match_case: bool = False
) -> dict:
    """
...
def main():
    ...
    find_parser = subparsers.add_parser(
        "find", help="Find presentations by search query"
    )
    ...
    add_slide_parser.add_argument(
        "--at", type=int, default=None, help="Insert position (0-based index)"
    )
    ...
    delete_slide_parser.add_argument(
        "slide_id", help="Slide object ID (from get-metadata)"
    )
    ...
    batch_parser.add_argument(
        "requests", help="JSON array of batch update request objects"
    )
    ...
    if args.command == "get-text":
        ...
        result = replace_text(
            args.presentation, args.find, args.replace, args.match_case
        )
