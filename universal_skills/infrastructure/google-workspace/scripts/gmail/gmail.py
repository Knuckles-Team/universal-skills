...
def api_request(
    ...
    try:
        ...
        payload, _ = open_json(
            req,
            policy=UrlPolicy(frozenset({"gmail.googleapis.com"})),
            timeout=30,
    ...
    if attachments:
        for filepath in attachments:
            ...
            if not os.path.isfile(filepath):
                raise FileNotFoundError("Configured attachment was not found")
...
def send_email(
    ...
    mime_message = create_mime_message(
        to, subject, body, cc, bcc, is_html, from_addr, attachments
    )
...
def create_draft(
    ...
    result = api_request(
        "POST", "users/me/drafts", data={"message": {"raw": mime_message}}
    )
...
def main():
    ...
    get_parser.add_argument(
        "--format",
        choices=["minimal", "full", "raw", "metadata"],
        default="full",
    ...
    send_parser.add_argument(
        "--from",
        dest="from_addr",
        help="Send from alias email address (must be configured in Gmail)",
    )
    send_parser.add_argument(
        "--attach",
        action="append",
    ...
    draft_parser.add_argument(
        "--html", action="store_true", help="Create as HTML email"
    )
    draft_parser.add_argument(
        "--from",
        dest="from_addr",
