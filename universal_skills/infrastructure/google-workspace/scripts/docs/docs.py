...
def api_request(
    method: str, url: str, data: Optional[dict] = None, params: Optional[dict] = None
) -> dict:
    ...
    try:
        ...
        payload, _ = open_json(
            req,
            policy=UrlPolicy(
                frozenset({"docs.googleapis.com", "www.googleapis.com"})
            ),
            timeout=30,
...
def extract_doc_id(doc_id_or_url: str) -> str:
    ...
    patterns = [r"docs\.google\.com/document/d/([a-zA-Z0-9_-]+)", r"^([a-zA-Z0-9_-]+)$"]
...
def create_doc(
    title: str, markdown: Optional[str] = None, folder_name: Optional[str] = None
) -> dict:
    ...
    if markdown:
        ...
        result = api_request(
            "POST", f"{DOCS_API_BASE}/documents", data={"title": title}
        )
        ...
        if doc_id and markdown:
            ...
            if "error" in insert_result:
                return {
                    "documentId": doc_id,
                    "title": title,
                    "warning": "Doc created but text insertion failed",
                }
...
def get_text(document_id: str) -> dict:
    ...
    params = {"fields": "title,body.content"}
    ...
    return {"documentId": doc_id, "title": result.get("title", ""), "text": text}
...
def append_text(document_id: str, text: str) -> dict:
    ...
    doc_result = api_request(
        "GET", f"{DOCS_API_BASE}/documents/{doc_id}", params=params
    )
    ...
    requests = [{"insertText": {"location": {"index": location_index}, "text": text}}]
...
def main():
    ...
    create_parser.add_argument(
        "--content", help="Initial content (plain text or markdown)"
    )
