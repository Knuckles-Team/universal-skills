...
CONFIG_LOCATIONS = [
    ...
    Path.home() / ".config" / "agent" / "mssql-connections.json",
]
MAX_ROWS = 10000
...
def is_read_only(query: str) -> bool:
    ...
    if re.match(r"^EXEC\s+SP_HELP\b", query_upper) or re.match(
        r"^SP_HELP\b", query_upper
    ):
        return True
    ...
    if limit and not re.search(r"\bTOP\s+\d+", query, re.IGNORECASE):
        ...
        if query.upper().strip().startswith("WITH"):
            ...
            if match:
                ...
                query = (
                    query[:pos]
                    + f"{original} TOP {limit}"
                    + query[pos + len(original) :]
                )
        else:
    ...
    try:
        ...
        with conn.cursor() as cursor:
            ...
            if cursor.description:
                ...
                # Print header
                header = " | ".join(
                    col[:MAX_COLUMN_WIDTH].ljust(widths[i])
...
Examples:
  ...
  %(prog)s --db mydb --query "SELECT * FROM users" --limit 100
    ...
    parser.add_argument(
        "--limit", "-l", type=int, help="Limit rows returned (uses TOP N)"
    )
