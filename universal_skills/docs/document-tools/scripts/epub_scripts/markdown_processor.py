...
class MarkdownProcessor:
    ...
    def _parse_chapters(self, content: str) -> List[Chapter]:
        ...
        if not h1_matches:
            ...
            if content.strip():
                ...
                chapters.append(
                    Chapter(title="Untitled", content="", sections=sections)
                )
            return chapters
        ...
        for i, match in enumerate(h1_matches):
            ...
            chapter_content = re.sub(
                r"__CODE_BLOCK_(\d+)__", restore_code_block, chapter_content_no_code
            ).rstrip()
            ...
            chapters.append(
                Chapter(title=title, content=direct_content, sections=sections)
            )
    ...
    def _parse_sections(self, content: str, min_level: int = 2) -> List[Section]:
        ...
        for i, match in enumerate(matches):
            ...
            sections.append(Section(title=title, level=level, content=section_content))
    ...
    def markdown_to_html(markdown_text: str) -> str:
        ...
        for line in lines:
            ...
            if line.strip().startswith("```"):
                if in_code_block:
                    ...
                    if current_paragraph:
                        ...
                        if paragraph_text:
                            html_parts.append(
                                f"<p>{MarkdownProcessor._render_inline(paragraph_text)}</p>"
                            )
                        current_paragraph = []
            ...
            if line.strip().startswith(">"):
                ...
                html_parts.append(
                    f"<blockquote>{MarkdownProcessor._render_inline(quote_text)}</blockquote>"
                )
                continue
            ...
            if line.startswith("#"):
                ...
                if level <= 6:
                    html_parts.append(
                        f"<h{level}>{MarkdownProcessor._render_inline(header_text)}</h{level}>"
                    )
                continue
            ...
            if line.strip().startswith(("- ", "* ")) or re.match(
                ...
                html_parts.append(
                    f"<li>{MarkdownProcessor._render_inline(list_item)}</li>"
                )
                continue
            ...
            for row in rows:
                ...
                for i, cell in enumerate(row):
                    if i < len(headers):  # Don't exceed header count
                        html_parts.append(
                            f"<td>{MarkdownProcessor._render_inline(cell)}</td>"
                        )
                # Fill missing cells
    ...
    @staticmethod
    def _add_line_numbers_and_highlighting(
