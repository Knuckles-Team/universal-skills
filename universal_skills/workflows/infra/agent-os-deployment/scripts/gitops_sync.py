...
class GitOpsSyncer:
    def __init__(
        self, gitlab_url, gitlab_token, portainer_url, portainer_token, endpoint_id="3"
    ):
    ...
    def parse_inventory(self, inventory_path):
        ...
        if not os.path.exists(inventory_path):
            logging.warning(
                f"Inventory file not found at {inventory_path}. Skipping node scanning."
            )
            return {}
        ...
        try:
            ...
            for group, gdata in all_groups.items():
                ...
                for host, hdetails in hosts.items():
                    ip = (
                        hdetails.get("ansible_host") or hdetails.get("ip")
                        if isinstance(hdetails, dict)
                        else host
                    )
                    nodes[host] = {"ip": ip, "group": group, "details": hdetails}
            logging.info(
                f"Discovered {len(nodes)} active hardware nodes from inventory."
            )
            return nodes
    ...
    def parse_workspace(self, workspace_path):
        ...
        if not os.path.exists(workspace_path):
            logging.warning(
                f"Workspace file not found at {workspace_path}. Skipping repository scanning."
            )
            return []
        ...
        try:
            ...
            logging.info(
                f"Discovered {len(repositories)} active workspaces / repositories."
            )
            return repositories
        except Exception as e:
    ...
    def run_git(self, cmd, cwd):
        ...
        if result.returncode != 0:
            logging.error(
                f"Git execution error: {' '.join(cmd)} in {cwd}. Error: {result.stderr}"
            )
            return False
        return True
    ...
    def sync_to_gitlab(self, local_path, service_name, gitlab_project_path):
        ...
        if res.status_code == 404:
            ...
            grp_res = requests.get(
                f"{self.gitlab_url}/api/v4/groups/{namespace_path}",
            ...
            create_res = requests.post(
                create_url, headers=self.gitlab_headers, json=payload
            )
        ...
        self.run_git(
            ["git", "config", "user.email", "deployer@agent-os.arpa"], local_path
        )

        self.run_git(["git", "add", "."], local_path)
        self.run_git(
            ["git", "commit", "-m", "chore: sync compose for GitOps deployment"],
            local_path,
        )
    ...
    def deploy_portainer_gitops(
        self, service_name, repo_url, compose_path="docker-compose.yml"
    ):
        logging.info(
            f"Querying active stacks to capture environment for {service_name}..."
        )
        stacks_res = requests.get(
            f"{self.portainer_url}/api/stacks", headers=self.portainer_headers
        )
        ...
        if target_stack:
            ...
            logging.info(
                f"Stack {service_name} (ID: {stack_id}) exists. Rebuilding stack..."
            )

            # Delete active stack
            del_res = requests.delete(
                f"{self.portainer_url}/api/stacks/{stack_id}?endpointId={self.endpoint_id}&external=false",
                headers=self.portainer_headers,
            )
        ...
        deploy_res = requests.post(
            swarm_stacks_url, headers=self.portainer_headers, json=payload
        )
...
def main():
    parser = argparse.ArgumentParser(
        description="Agent OS Scaled GitOps Synchronization Utility"
    )
    ...
    parser.add_argument(
        "--service-path",
        required=True,
        help="Local directory path containing service compose files",
    )
    parser.add_argument(
        "--service-name", required=True, help="Target service stack name"
    )
    parser.add_argument(
        "--gitlab-project",
        required=True,
        help="GitLab namespace project path, e.g. homelab/containers/services/twenty",
    )
    ...
    parser.add_argument(
        "--portainer-url", required=True, help="Portainer admin API URL"
    )
    parser.add_argument("--portainer-token", required=True, help="Portainer API key")
    parser.add_argument(
        "--endpoint-id", default="3", help="Portainer target Swarm/Docker endpoint ID"
    )
    ...
    repo_url = syncer.sync_to_gitlab(
        args.service_path, args.service_name, args.gitlab_project
    )
    if repo_url:
