...
def get_text(spreadsheet_id: str, output_format: str = "text") -> dict:
    ...
    metadata = api_request(
        SHEETS_API_BASE, f"spreadsheets/{sheet_id}", {"includeGridData": "false"}
    )
    ...
    for sheet in sheets:
        ...
        values_result = api_request(
            SHEETS_API_BASE,
            f"spreadsheets/{sheet_id}/values/{urllib.parse.quote(sheet_name)}",
        )
...
def get_range(spreadsheet_id: str, range_notation: str) -> dict:
    ...
    result = api_request(
        SHEETS_API_BASE,
        f"spreadsheets/{sheet_id}/values/{urllib.parse.quote(range_notation)}",
    )
...
def update_range(
    ...
    """
    sheet_id = extract_spreadsheet_id(spreadsheet_id)
    data = {"range": range_notation, "majorDimension": "ROWS", "values": values}
...
def main():
    ...
    get_range_parser.add_argument(
        "range", help="A1 notation range (e.g., 'Sheet1!A1:B10')"
    )
    ...
    update_range_parser.add_argument(
        "range", help="A1 notation range (e.g., 'Sheet1!A1:B2')"
    )
    update_range_parser.add_argument(
        "values", help='JSON 2D array of values (e.g., \'[["A1","B1"],["A2","B2"]]\')'
    )
    update_range_parser.add_argument(
        "--raw", action="store_true", help="Use RAW input (no formula parsing)"
    )
